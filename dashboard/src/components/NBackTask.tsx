 ...
 * Used to induce and measure known levels of cognitive load.
 *
 * Protocol:
...
function generateSequence(
  ...
  for (let i = 0; i < n; i++) {
    seq.push(randomLetter(seq[seq.length - 1]));
  }
...
export function NBackTask({
  ...
  // Prevent the same trial from being scored more than once.
  const trialScored = useRef(false);

  const finishTrial = useCallback(
    (didRespond: boolean) => {
      // Critical protection:
      // each trial can only be scored once.
      if (trialScored.current) return;

      trialScored.current = true;

      const target =
        currentIdx >= n &&
        sequence[currentIdx] === sequence[currentIdx - n];
      ...
      feedbackTimer.current = setTimeout(() => {
        ...
        if (next >= trials) {
          ...
          console.log("N-back scoring check:", {
            total,
            expected: trials,
            hits: hits.current,
          ...
          const accuracy =
            total > 0
              ? (hits.current + correctRejections.current) / total
              : 0;
    ...
    [currentIdx, n, trials, sequence, onComplete]
  );
  ...
  }, [phase, currentIdx, sequence, finishTrial]);
  ...
  if (phase === "instructions") {
    return (
      ...
      >
        ...
        >
          A letter will appear every 2 seconds. Press{" "}
          <kbd
            style={{
              background: "#f1f5f9",
              padding: "2px 8px",
          ...
          whenever the current letter matches the letter{" "}
          <strong>{n}</strong> step{n > 1 ? "s" : ""} ago.
        </p>
    ...
    return (
      ...
      >
        ...
        <p style={{ color: "#64748b" }}>
          Results submitted for calibration.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        {currentIdx >= n
          ? `Does this match "${sequence[currentIdx - n]}"?`
          : "Memorising…"}
      </p>
      ...
      >
        ...
        <span>
          ⚠️ False alarms: {falseAlarms.current}
        </span>
      </div>
    </div>
  );
